...
export default function SiteHeader() {
  ...
  const isAppRoute = activePath.startsWith("/dashboard") || activePath.startsWith("/learn") || activePath.startsWith("/lab") || activePath.startsWith("/tools") || activePath.startsWith("/progress");
  ...
  return (
    <header className="sticky top-0 z-30 backdrop-blur-lg bg-background/80 border-b border-border">
      <div className="mx-auto max-w-6xl px-4 md:px-6 h-16 flex items-center justify-between gap-4">
        {/* Logo */}
        <Link to={showAppMenu ? "/dashboard" : "/"} className="flex items-center gap-2 font-bold text-lg font-display shrink-0">
            ...
            <>
              ...
              <DropdownMenu>
                <DropdownMenuTrigger asChild>
                  <Button variant="ghost" className="rounded-full px-3 gap-1 hover:bg-muted font-normal h-8">
                    Tools <ChevronDown className="h-3.5 w-3.5 opacity-50" />
                  </Button>
                </DropdownMenuTrigger>
                <DropdownMenuContent align="start" className="w-48 rounded-2xl">
                  <DropdownMenuItem asChild><Link to="/tools/periodic-table">📊 Bảng tuần hoàn</Link></DropdownMenuItem>
                  <DropdownMenuItem asChild><Link to="/tools/explorer">🔍 Compound Explorer</Link></DropdownMenuItem>
                  <DropdownMenuItem asChild><Link to="/tools/molecules">📋 Thư viện phân tử</Link></DropdownMenuItem>
                  <DropdownMenuItem asChild><Link to="/tools/reactions">⚗️ Danh sách phản ứng</Link></DropdownMenuItem>
                </DropdownMenuContent>
              </DropdownMenu>
          ...
          {showAppMenu ? (
            <DropdownMenu>
              ...
              <DropdownMenuContent align="end" className="w-48 rounded-2xl">
                ...
                <DropdownMenuItem asChild><Link to="/progress">Tiến độ</Link></DropdownMenuItem>
                <DropdownMenuSeparator />
                {user ? (
                  <DropdownMenuItem onClick={() => supabase.auth.signOut()} className="text-red-500">
