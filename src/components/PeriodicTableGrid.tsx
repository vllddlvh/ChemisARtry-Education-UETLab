...
export default function PeriodicTableGrid({
  ...
  const isMatch = (e: PTElement) => {
    if (highlightCategory && e.category.toLowerCase() !== highlightCategory.toLowerCase())
      return false;
    if (query) {
  ...
  const renderCell = (e: PTElement | undefined, key: string) => {
    ...
    return (
      <div key={key} className={dim ? "opacity-25 transition" : "transition"}>
        <ElementCard element={e} selected={selectedNumber === e.number} onClick={onSelect} />
      </div>
    );
  };
        ...
        {(
          ...
          .map((cat) => {
            ...
            return (
              <div
                key={cat}
                className={`inline-flex items-center gap-1.5 rounded-full px-2.5 py-1 text-xs ${s.bg} ${s.text} ring-1 ${s.ring}`}
              >
