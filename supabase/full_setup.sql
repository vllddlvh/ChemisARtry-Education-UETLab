-- =====================================================================
-- MoleLab AR — full database setup
-- Run this once in the Supabase SQL editor (or psql) to bootstrap a
-- brand-new project. Safe to re-run: drops the existing public tables.
-- =====================================================================
...
-- Molecules and reactions are public reference data: anyone can read.
CREATE POLICY "Molecules public read"
  ON public.molecules FOR SELECT USING (true);

CREATE POLICY "Reactions public read"
  ON public.reactions FOR SELECT USING (true);

-- User progress: each user can only see and modify their own row.
...
-- =====================================================================
-- SEED: MOLECULES
-- atoms : [{el, x, y, z}]   (Ångström-ish, tuned for ball-and-stick)
-- bonds : [{a, b, order}]   (a, b are atom indices, order 1/2/3)
-- =====================================================================
...
-- =====================================================================
-- SEED: REACTIONS
-- reactants / products use formulas matching public.molecules.formula
-- so the AR scene can spawn product molecules.
-- energy_kj is approximate ΔH in kJ (negative = exothermic).
-- =====================================================================

INSERT INTO public.reactions (reactants, products, equation, description, energy_kj) VALUES
-- Original reactions
(ARRAY['H2','O2'], ARRAY['H2O'], '2 H₂ + O₂ → 2 H₂O',
  'Combustion of hydrogen — the classic formation of water. Highly exothermic.', -571.6),
(ARRAY['Na','Cl2'], ARRAY['NaCl'], '2 Na + Cl₂ → 2 NaCl',
  'Synthesis of table salt from its elements. Violent and bright reaction.', -822.0),
(ARRAY['CH4','O2'], ARRAY['CO2','H2O'], 'CH₄ + 2 O₂ → CO₂ + 2 H₂O',
  'Combustion of methane — burning natural gas releases energy.', -890.0),
(ARRAY['NH3','O2'], ARRAY['H2O','N2'], '4 NH₃ + 3 O₂ → 2 N₂ + 6 H₂O',
  'Oxidation of ammonia (simplified). Industrial precursor to nitric acid.', -1267.0),
(ARRAY['H2','Cl2'], ARRAY['HCl'], 'H₂ + Cl₂ → 2 HCl',
  'Formation of hydrogen chloride from its elements.', -184.0),
